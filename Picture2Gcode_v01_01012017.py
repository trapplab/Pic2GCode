			...
			if enden:							#wenn anfangspunkt dann ....
				...
				while pos < enden[1]  and pos > enden[0]:
					if nextPoint(BildMatrix[line-1,:],pos, vonRechts):
						...
						if vonRechts:
							gcode_file.write(whichGCommand(BildMatrix[line-1,pos-1]) + " X" + getX(pos-2)+ " Y" + getY(BildMatrix.shape[0]+1-line) + getF(BildMatrix[line-1,pos-1]) + "\n\n")	#bei farbÃ¤nderung zur Grenze fahren
						else:
							gcode_file.write(whichGCommand(BildMatrix[line-1,pos-1]) + " X" + getX(pos)+ " Y" + getY(BildMatrix.shape[0]+1-line) + getF(BildMatrix[line-1,pos-1]) + "\n\n")	#bei farbÃ¤nderung zur Grenze fahren
						##
						if vonRechts:
							pos = pos-1
						else:
...
def getM03(pix):
	...
	if pix:
		return "M03 S" + str(int(round(pix*staerkeStep+staerke[0])))
	else:
		return "M05"
